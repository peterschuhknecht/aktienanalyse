...
5. Pflicht-Frischkurs: fuer JEDE aktiv bearbeitete Aktie zuerst den aktuellen Kurs frisch holen - primaer via `WebFetch` auf die Yahoo-Chart-JSON (`05_data/quote_source_endpoints.md`; `regularMarketPrice`/`chartPreviousClose`/`regularMarketTime`, Fallback-Kette dort), Websuche nur Fallback/Datenluecke; bei Re-Abruf im selben Lauf URL variieren (15-Min-Cache). Dann uebrige frische News/Fundamentaldaten; alles in CSV speichern. CSV-Kurse sind nur Seeds.
